...
Apple Silicon / macOS 26+ 메뉴바 앱. `RetroStats/` 아래 AppKit·SwiftUI Swift 소스를 `RetroStats.xcodeproj`에서 `xcodebuild`로 빌드한다. 외부 패키지는 없다.
...
- 문서만 수정하면 경로·명령의 정확성을 확인한다.
- 회귀 검사는 `--self-test`로 실행되는 `RetroStats/Testing/SelfTest.swift`, `Storage/StorageSelfTest.swift`, `Storage/DerivedDataSelfTest.swift`, `Dashboard/DashboardSelfTest.swift`, `Transition/TransitionSelfTest.swift`에 있다. 동작 변경 시 관련 회귀 검사를 유지·보강한다.
